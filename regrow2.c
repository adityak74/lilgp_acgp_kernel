...
int operator_regrow2_init ( char *options, breedphase *bp )
{
    ...
    if (p=get_parameter( "acgp.level")) {
        // printf("We are using level 2 heuristics operator regrow2.\n");
         Acgp_level = atof(p);
    }
    else {
         Acgp_level = 1;
     ...
     if (Acgp_level < 2) {
     	printf("Trying to use level 2 heuristic operator(regrow2) when acgp_level is not greater than 1!\n");
	...
	exit(1);
     ...
     bp->operator = OPERATOR_REGROW2;
     bp->data = (void *)md;
     bp->operator_free = operator_regrow2_free;
     bp->operator_start = operator_regrow2_start;
     bp->operator_end = operator_regrow2_end;
     bp->operator_operate = operator_regrow2;
...
void operator_regrow2_free ( void *data )
{
     regrow_data * md;
...
void generate_random_full_tree_lvl1_lcl1_mwh ( int space, int depth, function_set *fset,int i )
{    int j;
     int save;

     // Allocate an array to hold children of 'i' so we can use it to store
     // children generated by our spinning of local information
     int *child_list_lcl;
...
void generate_random_grow_tree_lvl1_lcl1_mwh ( int space, int depth, function_set *fset , int i)
{    int j;
     int save;

     // Allocate an array to hold children of 'i' so we can use it to store
     // children generated by our spinning of local information
     // Obviously only do it if this isnt a terminal
     int *child_list_lcl;
...
void generate_random_full_tree_wrapper_lvl1_lcl1_mwh(int space, int depth, function_set *fset,const int source) 
/*
...
*/
{
	...
	if (source == REGROW_SOURCE) {
...
#ifdef DEBUG_REGROW
		printf("Generated after conversion (%d),[",i);
		...
		int trueIndex = MS_czj[NumFT][0].convert_to_ms[i];
		...
		{ case FUNC_DATA: case EVAL_DATA:
			...
			{ Function_czj = i;
			  Argument_czj = j;
			  generate_random_full_tree_wrapper_lvl1_lcl1_mwh(space,depth-1,fset,WRAPPER_SOURCE);
			}
			break;
		   case FUNC_EXPR: case EVAL_EXPR:
		   	for (j = 0; j < (fset->cset)[i].arity; ++j)
...
void generate_random_grow_tree_wrapper_lvl1_lcl1_mwh(int space, int depth, function_set *fset,const int source) 
/*
 Depth is the depth left until the bottom
*/
{
	...
	if (source == REGROW_SOURCE) {
		...
		{ case FUNC_DATA: case EVAL_DATA:
			...
			{ Function_czj = i;
			  Argument_czj = j;
			         generate_random_grow_tree_wrapper_lvl1_lcl1_mwh(space,depth-1,fset,WRAPPER_SOURCE);
			}
			break;
		   case FUNC_EXPR: case EVAL_EXPR:
		   	for (j = 0; j < (fset->cset)[i].arity; ++j)
...
void operator_regrow2 ( population *oldpop, population *newpop,
                      void *data )
{
     int i;
     int ps;
     ...
     while(1)
     {
	  ...
	  ns = tree_depth_to_subtree ( oldpop->ind[p].tr[t].data,
                    ...
                    tree_depth ( gensp[1].data );
	  FILE *fp;
	  fp=fopen("regrow2growdepths.dat","a+");
	  fprintf(fp,"%d\n",ns);
	  fclose(fp);
#endif
	  ...
	  /* check the regrowd tree against node count and/or size limits. */
		...
		// checks for trees greater than 4 
		//printf("ns=%d ",ns);   
		//if (ns > 4) {
			//fprintf ( stderr, "the new subtree that has over 200 nodes is: " );
          	//	print_tree ( gensp[1].data, stderr );
			//exit(1);
		//}    
          }
...
/************ end czj changes */
  ...
  { oprintf(OUT_SYS, 10, "INVALID TREE in regrow2\n");
    exit(1);
  }
#endif
