...
int operator_mutate2_init ( char *options, breedphase *bp )
{
     int errors = 0;
     mutate_data *md;
     int i, j, k, m;
    ...
    //     printf("We are using level 2 heuristics operator mutation2.\n");
         Acgp_level = atof(p);
    }
    else {
         Acgp_level = 1;
     ...
     if (Acgp_level < 2) {
     	printf("Trying to use level 2 heuristic operator(mutation2) when acgp_level is not greater than 1\n");
	...
	exit(1);
     ...
     bp->operator = OPERATOR_MUTATION2;
     bp->data = (void *)md;
     bp->operator_free = operator_mutate2_free;
     bp->operator_start = operator_mutate2_start;
     bp->operator_end = operator_mutate2_end;
     bp->operator_operate = operator_mutate2;
...
void operator_mutate2_free ( void *data )
{
     mutate_data * md;
...
int spin_mutation_wheel_lvl1_lcl1_for_lvl1(int trueIndex,int *context,int fixed)
/*
...

{
	...
	for (i = 0; i < farity; i++) {
		if (i != fixed) {
			context[i] = MS_czj[trueIndex][i].convert_to_ms[context[i]];
		}
	}
...
// if we only want root as a function, we need to do that 
int spin_mutation_wheel_lvl1_lcl1_for_lvl1_function(int trueIndex,int *context,int fixed)
/*
   'context' is an array of children of the root 'trueIndex'
   'fixed' is the part that we are mutating from
*/

{
	...
	// number of functions that will be on the wheel
	// since we only want functions
	int size_fixed = MS_czj[trueIndex][fixed].numF;
	
	
	// NO legal mutates that give functions
	// so take any mutation that we can
	if (size_fixed == 0) {
		return spin_mutation_wheel_lvl1_lcl1_for_lvl1(trueIndex,
		context,fixed);	
	}
...
#ifdef DEBUG_MUTATE
	printf("Spinning wheel for arity %d\n",fixed);
	printf("Only want function here\n");
	printf("Wheel is [");
	for (i = 0; i < size_fixed; i++) {
		printf("%f ",mutation_wheel[i]);
	}
...
int spin_mutation_wheel_lvl1_lcl1_for_lvl1_terminal(int trueIndex,int *context,int fixed)
/*
   'context' is an array of children of the root 'trueIndex'
   'fixed' is the part that we are mutating from
*/

{
	...
	// number of terminals that will be on the wheel
	// since we only want terminals
	int size_fixed = MS_czj[trueIndex][fixed].numT;
	
	// NO legal mutates that give terminals
	// so take any mutation that we can
	if (size_fixed == 0) {
		return spin_mutation_wheel_lvl1_lcl1_for_lvl1(trueIndex,
		context,fixed);	
	...
	// store where the terminals start
	...
	for (i = 0; i < size_fixed; i++) {
		this_offset = base_offset +
					// (i+start_terminals to take those weights past the functions
			Level1_Cnt_mwh.f[f].offset[fixed]*(i+start_terminals);
	...
	// now increment winner to point at the correct one
...
#ifdef DEBUG_MUTATE
	printf("Spinning wheel for arity %d\n",fixed);
	printf("Only want terminal here\n");
	printf("Wheel is [");
	for (i = 0; i < size_fixed; i++) {
		printf("%f ",mutation_wheel[i]);
	}
...
int spin_mutation_wheel_lvl1_lcl1_for_lcl1(int trueIndex,int *context,int fixed)
/*
   'context' is an array of children of the root 'trueIndex'
...
#ifdef DEBUG_MUTATE
	printf("%d converted to lcl1 counter index of %d\n",trueIndex,f);
...
#ifdef DEBUG_MUTATE
	printf("Spinning local info mutation wheel for arity %d, root %d\n",fixed,f);
	printf("Wheel is [");
	for (i = 0; i < size_fixed; i++) {
		printf("%f ",mutation_wheel[i]);
	}
...
// Only generate a terminal here
int spin_mutation_wheel_lvl1_lcl1_for_lcl1_terminal(int trueIndex,int *context,int fixed)
/*
   'context' is an array of children of the root 'trueIndex'
   'fixed' is the child(a arity) that we are mutating from
*/

{
	...
	// so take any mutation that we can
	if (size_fixed == 0) {
		return spin_mutation_wheel_lvl1_lcl1_for_lcl1(trueIndex,
		context,fixed);	
	}
...
#ifdef DEBUG_MUTATE
	printf("Spinning local info mutation wheel for arity %d, root %d\n",fixed,f);
	printf("Only want terminal here in spin_mutation_wheel_lvl1_lcl1_for_lcl1_terminals\n");
	printf("Wheel is [");
	for (i = 0; i < size_fixed; i++) {
		printf("%f ",mutation_wheel[i]);
	}
...
int spin_mutation_wheel_lvl1_lcl1_for_lcl1_function(int trueIndex,int *context,int fixed)
/*
   'context' is an array of children of the root 'trueIndex'
   'fixed' is the child(a arity) that we are mutating from
*/
...
void operator_mutate2_start ( population *oldpop, void *data )
{
     mutate_data * md;
...
void operator_mutate2_end ( void *data )
{
     mutate_data * md;
...
void operator_mutate2 ( population *oldpop, population *newpop,
                      void *data )
{
     int i,j;
     int ps;
	...
	fflush(stderr);
     ...
     while(1)
     {
	  ...
	  //printf("Md->internal=%f, MD->external=%f",md->internal,md->external);
	 	  
#ifdef DEBUG_MUTATE
	  printf("------------------------------\n");
	  printf("Md->internal=%f, MD->external=%f",md->internal,md->external);
	  printf("Md->mindepth=%d,Md->maxdepth=%d\n",md->maxdepth,md->mindepth);
	  printf("Entire tree is\n");
	  print_tree ( oldpop->ind[p].tr[t].data, stderr );
	  ...
	  printf("Depth of our mutation(replace_depth) is %d\n",replace_depth);
	  printf("Arity of replacement is %d\n",thisArity);
	  if (replace_depth > 0)
	  {
	  ...
	  if (replace_depth == 0) {
		...
		if (md->absDepth_czj)
			MinDepth_czj = depth - md->mindepth;
          	...
          	{
             	case GENERATE_GROW:
               		generate_random_grow_tree_wrapper_lvl1_lcl1_mwh ( 1, depth, fset+tree_map[t].fset,REGROW_SOURCE );
               		break;
                case GENERATE_FULL:
               		generate_random_full_tree_wrapper_lvl1_lcl1_mwh ( 1, depth, fset+tree_map[t].fset,REGROW_SOURCE );
               		break;
                case GENERATE_HALF_AND_HALF:
               		if ( random_double() < 0.5 )
                    		generate_random_grow_tree_wrapper_lvl1_lcl1_mwh ( 1, depth, fset+tree_map[t].fset,REGROW_SOURCE );  // calls mwh version for lvl 1 information
               		else
                    		generate_random_full_tree_wrapper_lvl1_lcl1_mwh ( 1, depth, fset+tree_map[t].fset,REGROW_SOURCE );  // calls wrapper for lvl 1 information
               		break;
          	}
	  ...
	  if (replace_depth == 1) {
		...
		if (md->absDepth_czj)
	  	...
	  	// If we are in here, we want to spin our wheel given our info
		// depending on our grow method
		switch(md->method)
		{
			case GENERATE_GROW:
				if (depth <= 0)
					i = spin_mutation_wheel_lvl1_lcl1_for_lvl1_terminal(trueIndex,level1context_mwh,thisArity);
				else if (depth>MinDepth_czj) {
					i = spin_mutation_wheel_lvl1_lcl1_for_lvl1_function(trueIndex,level1context_mwh,thisArity);
			             //MinDepth_czj--;  
				}	
				else
					i = spin_mutation_wheel_lvl1_lcl1_for_lvl1(trueIndex,level1context_mwh,thisArity);
			break;
			case GENERATE_FULL:
				if (depth <= 0)
					i = spin_mutation_wheel_lvl1_lcl1_for_lvl1_terminal(trueIndex,level1context_mwh,thisArity);
		...
		if (i < fset->function_count)
		     ...
		     // TODO: Make sure we should subtract depth here
		     depth = depth - 1;
	             switch(md->method)
		     {
		     case GENERATE_GROW:
				...
				{ case FUNC_DATA: case EVAL_DATA:
					...
					{ Function_czj = i;
					  ...
					  switch ( md->method )
          				  {
             	                          case GENERATE_GROW:
					       generate_random_grow_tree_lvl1_lcl1_mwh(space,depth-1,fset,child_list_lcl[j]);
					       break;
					  case GENERATE_FULL:
					       generate_random_full_tree_lvl1_lcl1_mwh(space,depth-1,fset,child_list_lcl[j]);
					       break;     
					  case GENERATE_HALF_AND_HALF:
               					if ( random_double() < 0.5 )   
						     generate_random_grow_tree_lvl1_lcl1_mwh(space,depth-1,fset,child_list_lcl[j]);
	  ...
	  if (replace_depth > 1) 
	  {
		  ...
		  switch( md->method)
		  {
		  	case GENERATE_GROW:
		  		if (depth <= 0)
		  			i = spin_mutation_wheel_lvl1_lcl1_for_lcl1_terminal(trueIndex,level1context_mwh,thisArity);
		  		else if (depth>MinDepth_czj)
		  	  	        i = spin_mutation_wheel_lvl1_lcl1_for_lcl1_function(trueIndex,level1context_mwh,thisArity);  
				else
					i = spin_mutation_wheel_lvl1_lcl1_for_lcl1(trueIndex,level1context_mwh,thisArity);
		  		break;
		  ...
		  if (i < fset->function_count)
		  {
			...
			// Subtract one as we have generated a level already
			depth = depth - 1;
			switch (md->method)
			{
				case GENERATE_GROW:
				...
				{ case FUNC_DATA: case EVAL_DATA:
					...
					{ Function_czj = i;
					  ...
					  switch ( md->method )
	          				{
       		      			        case GENERATE_GROW:
               				          generate_random_grow_tree_lvl1_lcl1_mwh (space, depth-1,
						  fset+tree_map[t].fset,child_list_lcl[j] );
               		              		  break;
             	 				case GENERATE_FULL:
                                                  generate_random_full_tree_lvl1_lcl1_mwh ( space, depth-1,
						  fset+tree_map[t].fset,child_list_lcl[j] );
               		                          break;
                                                case GENERATE_HALF_AND_HALF:
               		                          if ( random_double() < 0.5 )
...
/************ begin czj changes */
            ...
            if (md->absDepth_czj && tree_depth( gensp[1].data ) < md->mindepth) {
              badtree = 1;
	      printf("Setting badtree, will duplicate individual, Tree too short, generated %d\n",tree_depth( gensp[1].data ));
	     }
/************ end czj changes */
