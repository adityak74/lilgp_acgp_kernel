/* Operators for lvl1 and lvl2 information */
...
void generate_crossover_weights_lvl1_mwh(int trueIndex,int *context,int fixed)
/*
...
int spin_after_generating_crossover_weights_lvl1_mwh(lnode *l, int size_tree,int trueIndex, int fixed) 
/*
...
*/
{
	...
	int cur_node=0;
        generate_crossover_wheel_recurse_lvl1(&l,crossover_wheel,&cur_node,trueIndex,fixed);
...
int generate_crossover_wheel_recurse_lvl1( lnode **l,double *crossover_wheel, int *cur_node , int trueIndex, int fixed)
{
     function *parent;
...
int operator_crossover2Global_init ( char *options, breedphase *bp )
{
    ...
    if (p=get_parameter( "acgp.level")) {
         printf("We are using level 2 heuristics operator crossover2Global.\n");
         Acgp_level = atof(p);
    }
    else {
         Acgp_level = 1;
     ...
     if (Acgp_level < 2) {
     	printf("Trying to use level 2 heuristic operator(crossover2Global) when acgp_level is not less than 2!\n");
	...
	exit(1);
     ...
     bp->operator = OPERATOR_CROSSOVER2GLOBAL;
     bp->data = (void *)cd;
     bp->operator_free = operator_crossover2Global_free;
     bp->operator_start = operator_crossover2Global_start;
     bp->operator_end = operator_crossover2Global_end;
     bp->operator_operate = operator_crossover2Global;
...
void operator_crossover2Global_free ( void *data )
{
     crossover_data * cd;
...
void operator_crossover2Global ( population *oldpop, population *newpop,
                         void *data )
{ crossover_data * cd;
  int p1, p2;
  int ps1, ps2;
...
printf("Parent1 has %d nodes, parent2 has %d nodes\n",ps1,ps2); /* czj debug */
  ...
  { 
      ...
      if (ps1 > 1) {
       ...
       if (replace_depth == 1) {
	    ...
	    // fset's function 3's weight given the context information
       	    generate_crossover_weights_lvl1_mwh(trueIndex,level1context_mwh,thisArity);
       }
    ...
    if (replace_depth == 1) {
       ...
       int num_in_source = spin_after_generating_crossover_weights_lvl1_mwh(oldpop->ind[p2].tr[t2].data,ps2,trueIndex,thisArity);
  ...
  { 
	...
	// Only gather info if it has more than just a root, that is has depth 0
	 ...
	 // Now select our dest from our own wheel, so spin it again
      	 ...
      	 // zero for the next time
     	  //TODO: Perhaps only let it take all nodes or just internal nodes
     	  int num_in_source = spin_after_generating_crossover_weights_lvl1_mwh(oldpop->ind[p1].tr[t1].data,ps1,trueIndex,thisArity);
