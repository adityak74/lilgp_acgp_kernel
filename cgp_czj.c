...
/*     that is this prct of all trees will be taken              */
/*     alltogether from all subpopulations                       */
/*   acgp.select_all  [0,1]                                      */
/*     1 - take trees from each subpopulation                    */
/*     0 - take best trees across subpopulations                 */
/*       always acgp.use_trees_prct is the effective rate        */
...
/*         withe rate proportional to generation number          */
...
/*   acgp.level [1,2,3]                                          */
/*     1 - no 2nd order heuristics                               */
/*     2 - process 2nd order heuristics and use in *2 operators  */
/*         and write 2nd order heuristics files                  */
/*     3 - as 2 but do not write the files                       */
...
/* Global setting to determine if we use level 2 heuristics, default is 1 */
int Acgp_level;
...
{ printf("\n\n\t\tWELCOME TO acgp/lilgp 1.2/1.02\n");
  printf("\n\t\tdeveloped by\n");
  printf("\tCezary Z. Janikow\n");
  printf("\tUniversity of Missouri - St. Louis\n");
  printf("\temailto:janikow@umsl.edu\n");
...
*/
  ...
  if (p=get_parameter( "acgp.level")) {
         Acgp_level = atof(p);
	 //if (Acgp_level > 1)
	 //	printf("We are using level 2 heuristics.\n");
  }
  else {
        // printf("We are not using level 2 heuristics.\n");
         Acgp_level = 1;
  }
  ...
  if (Acgp_level > 1)
      create_mwh();   // Generate lvl1 counters mwh
  printf("\nWheels are...\n"); 
   ...
   but must walk due to tree representation */ 
{
     ...
     /* mwh : Set up our depth counters and context information */
     depth_mwh = 0;
     size_context1_mwh = 0;
     size_context2_mwh = 0;
#ifdef DEBUG_LVL1
    printf("Gathering lvl 1 info\n\n");
#endif
#ifdef DEBUG_LCL1
	printf("Storing lvl1 and lcl1 statistics on tree:\n");
	print_tree(c,stderr);
	printf("Now recursing through tree, getting");
	printf("(%d)(",parIndex);
	fflush(stdout);
#endif
...
#ifdef DEBUG_LCL1
	printf(")\n");
	printf("Showing level1context_mwh,(%d)[",parIndex);
	...
	printf("]\n");
     ...
     if (Acgp_level > 1) {
        ...
        inc_counter_with_context_mwh(parIndex,expressed_count);	// update lvl2 information mwh
...
{    function *parent;
     ...
     depth_mwh++;
     if (Acgp_level > 1)
         update_context_mwh(depth_mwh,(**l).f->index); /* mwh update context arrays depending on depth */
         
     toUse = toUse && expressed_count;
     parent = (**l).f;
     parIndex=parent->index;
     
     // mwh: allocate child list of this parent
     int *child_list_mwh;   //mwh
     if (Acgp_level > 1 )
     		child_list_mwh = (int *)MALLOC(parent->arity * sizeof(int));  // mwh
...
#endif
	       if (Acgp_level > 1)
	       		child_list_mwh[i] = (**l).f->index;     //mwh: save child[i] of our parent
	       chIndex=(**l).f->index;
	...
	printf("%d,",(**l).f->index);
     ...
     // Dont store if this is lvl 1 info, will be stored in lvl1 counters
     if (depth_mwh != 1 && Acgp_level > 1)	//mwh
     	inc_local_level1_mwh(parent->index,child_list_mwh,parent->arity,expressed_count); //mwh

     if (Acgp_level > 1)		
     	FREE(child_list_mwh);  //mwh
...
/* called after every gp generation
   ...
   - else sort and select (int)ceil(popSize*acgp_internal_use_trees_prct)
...
{ int i,a,j;
  ...
  { char fname[BUFSIZ];
    ...
    if (acgp_select_all)
      acgp_internal_use_trees_prct=acgp_use_trees_prct/numPops;
    else
    ...
    { if (p=get_parameter( "acgp.gen_start_prct" ))
      ...
      if (p=get_parameter( "acgp.level")) {
         Acgp_level = atof(p);
	// if (Acgp_level > 1) 
	 	//printf("We are gathering level 2 heuristics.\n");
      }
      else {
         //printf("We are not using level 2 heuristics.\n");
         Acgp_level = 1;
    ...
    fprintf(fp,"\trandom_seed=%d\n", !(get_parameter("random_seed")) ? 
                 1 : (atoi(get_parameter("random_seed"))));
  ...
  { if (!((curGen-acgp_gen_start)%acgp_gen_step))
    { Acgp_adj_regrow=1;
      if (acgp_gen_slope==1)
        acgp_prctChnge= (double)(curGen+1)/(maxGen+1);
      if (acgp_what==3)
        Acgp_adj_regrow=2;
    }
  }
...
/* remove to avoid printing the whole population if want to ****************/
  ...
  #ifdef DEBUG_LVL1
  if (Acgp_level > 1)
      list_counted_level1_mwh(curGen,basefile,0);
  #endif
/***************************************** end remove **********************/
...
#ifdef DEBUG_LVL1
	// Stores in .gcnt file the total quality trees we are storing on
   if (Acgp_level > 1)
      list_counted_level1_mwh(curGen,basefile,0);
#endif
  if (Acgp_level == 2) 
      print_cnts_mwh(curGen,basefile,Acgp_adj_regrow>0);  
                                                        /* on every generation */

  if (Acgp_adj_regrow<1)                   /* no adjustments this generation */
  { acgp_print_wghts(curGen,fp,Acgp_adj_regrow>0);
     if (Acgp_level == 2)
         print_weights_mwh(curGen,basefile,Acgp_adj_regrow>0);
  ...
  if (Acgp_level > 1)
       adjust_weights_with_counters_mwh(acgp_prctChnge,acgp_0_threshold_prct); 
