...
#ifdef DEBUG_TIMING
     clock_t start_op, total_op = 0;
     clock_t start_regrow, total_regrow = 0;
#endif
...
********** czj remove code end, start new code ****************************************/
		 ...
		 // Timing for total time spent in operators...
#ifdef DEBUG_TIMING
		 start_op = clock();
#endif		 
...
#ifdef DEBUG_TIMING		 
		 total_op += ( clock() - start_op);   
#endif	 
		    
               } else { /* regrowing in this generation */
	         oprintf (OUT_SYS, 10, "regrow population...\n");
		 
		 // Timing for total time spent regrowing population...
#ifdef DEBUG_TIMING
		 start_regrow = clock();
#endif
...
#ifdef DEBUG_TIMING
		 total_regrow += ( clock() - start_regrow);
#endif   
...
/************************************************* czj new code end *******************/
     ...
     printf("------- timing -------\n");
     printf("\tWheelGen \t %.3fs\n", (float)total_gather/CLOCKS_PER_SEC);	
     printf("\tOperators \t %.3fs\n",(float)total_op/CLOCKS_PER_SEC);
     printf("\tTotal both \t %.3fs\n",(float)(total_gather+total_op)/CLOCKS_PER_SEC);
     
     printf("\tJust regrow \t %.3fs\n",(float)total_regrow/CLOCKS_PER_SEC);
     printf("\tTotal all \t %.3fs\n",(float)(total_gather+total_op+total_regrow)/CLOCKS_PER_SEC);
#endif
     ...
     if ( gen%stt_interval == 0 )
     {
          if ( test_detail_level ( 50 ) )
          {
               ...
               oprintf ( OUT_STT, 50, "%.*lf %.*lf %.*lf ",
                        ...
                        fd, gen_stats[0].bestfit, 
//			fd, 2048-1/gen_stats[0].bestfit,
			fd, gen_stats[0].worstfit );
               oprintf ( OUT_STT, 50, "%.3lf %.3lf %d %d %d %d ",
                        (double)gen_stats[0].totalnodes/gen_stats[0].size,
                        (double)gen_stats[0].totaldepth/gen_stats[0].size,
                        gen_stats[0].bestnodes, gen_stats[0].bestdepth,
...
#ifdef DEBUG_TIMING		 
		 total_gather += ( clock() - start_gather);   
#endif
