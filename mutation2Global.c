...
int operator_mutate2Global_init ( char *options, breedphase *bp )
{
    ...
    if (p=get_parameter( "acgp.level")) {
         printf("We are using level 2 heuristics operator mutation2Global.\n");
         Acgp_level = atof(p);
    }
    else {
         Acgp_level = 1;
     ...
     if (Acgp_level < 2) {
     	printf("Trying to use level 2 heuristic operator(mutation2Global) when acgp_level is not greater than 1!\n");
	...
	exit(1);
     ...
     bp->operator = OPERATOR_MUTATION2GLOBAL;
     bp->data = (void *)md;
     bp->operator_free = operator_mutate2Global_free;
     bp->operator_start = operator_mutate2Global_start;
     bp->operator_end = operator_mutate2Global_end;
     bp->operator_operate = operator_mutate2Global;
...
void operator_mutate2Global_free ( void *data )
{
     mutate_data * md;
...
int spin_mutation_wheel_lvl1(int trueIndex,int *context,int fixed)
/*
...
int spin_mutation_wheel_lvl1_function(int trueIndex,int *context,int fixed)
/*
   'context' is an array of children of the root 'trueIndex'
   'fixed' is the part that we are mutating from
*/
...
int spin_mutation_wheel_lvl1_terminal(int trueIndex,int *context,int fixed)
/*
   'context' is an array of children of the root 'trueIndex'
   'fixed' is the part that we are mutating from
*/
...
void operator_mutate2Global ( population *oldpop, population *newpop,
                      void *data )
{
     int i,j;
     int ps;
     ...
     while(1)
     {
	  ...
	  if (replace_depth == 1) {
		...
		switch(md->method) {
			case GENERATE_GROW:
				if (depth <= 0)
					i = spin_mutation_wheel_lvl1_terminal(trueIndex,level1context_mwh,thisArity);
				else if (depth>MinDepth_czj)
					i = spin_mutation_wheel_lvl1_function(trueIndex,level1context_mwh,thisArity);
				else
					i = spin_mutation_wheel_lvl1(trueIndex,level1context_mwh,thisArity);
				break;
			case GENERATE_FULL:
				if (depth <= 0)
					i = spin_mutation_wheel_lvl1_terminal(trueIndex,level1context_mwh,thisArity);
		...
		//printf("The root of our winning mutate tree is %d\n",i);
		int save;
		int space = 1;
				...
				{ case FUNC_DATA: case EVAL_DATA:
					...
					{ Function_czj = i;
					  ...
					  switch ( md->method )
	                                  {
       		                            case GENERATE_GROW:
               		                      generate_random_grow_tree ( space, depth-1, fset+tree_map[t].fset);
               		                      break;
             	                            case GENERATE_FULL:
	  ...
	  if (replace_depth > 1) {
		  ...
		  // At this point we want to have context information, a number 'l' indicating
		  // our mutation point and the depth of 'l' (that is, do we even need to use our lvl1 info)
		  
       		   depth = md->mindepth + random_int ( md->maxdepth - md->mindepth + 1 );
		   if (md->absDepth_czj)
		   	MinDepth_czj=depth-md->mindepth;
		  // This picks how deep we want to grow, this depth IS NOT the depth
		  // of our mutation point
