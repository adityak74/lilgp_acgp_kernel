...
/**** czj modifications:
   ...
   also, computes timing info (clock) and prints into .time file
*******/
...
/***** czj added ***************************/
     float timeStart_czj, timeRun_czj;;
     timeStart_czj = clock()/CLOCKS_PER_SEC;
/***** czj end *****************************/
...
#ifdef TIME_CZJ
     create_MS_czj(); 
     char *basename_czj=get_parameter("output.basename");
     char *ftimeName_czj=calloc(strlen(basename_czj)+strlen(".time")+1,sizeof(char));
     strcpy(ftimeName_czj,basename_czj);
     strcat(ftimeName_czj,".time");
     FILE *ftime_czj=fopen(ftimeName_czj,"w");
     if (!ftime_czj) {
...
#ifdef TIME_CZJ
     timeRun_czj=clock()/CLOCKS_PER_SEC-timeStart_czj;
     fprintf(ftime_czj,"Time[sec] %.3f\n",timeRun_czj);
     fclose(ftime_czj);
#endif
     return 0;
}
