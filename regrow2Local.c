...
int operator_regrow2Local_init ( char *options, breedphase *bp )
{
    ...
    if (p=get_parameter( "acgp.level")) {
         printf("We are using level 2 heuristics operator regrow2Local.\n");
         Acgp_level = atof(p);
    }
    else {
         Acgp_level = 1;
     ...
     if (Acgp_level < 2) {
     	printf("Trying to use level 2 heuristic operator(regrow2Local) when acgp_level is not less than 1!\n");
	...
	exit(1);
     ...
     bp->operator = OPERATOR_REGROW2LOCAL;
     bp->data = (void *)md;
     bp->operator_free = operator_regrow2Local_free;
     bp->operator_start = operator_regrow2Local_start;
     bp->operator_end = operator_regrow2Local_end;
     bp->operator_operate = operator_regrow2Local;
...
void operator_regrow2Local_free ( void *data )
{
     regrow_data * md;
...
void generate_random_full_tree_lcl1_mwh ( int space, int depth, function_set *fset,int i )
{    int j;
     int save;

// TODO: This needs to be taken care of a level above and passed
...
//       i=random_F_czj();          /* try function, but not guaranteed either */
     ...
     // TODO: Now we know our current parent, and to do this fully again, we want to
     // again spin our local information and find a winner, so again allocate our information
     // and deallocate at end			      
...
void generate_random_grow_tree_lcl1_mwh ( int space, int depth, function_set *fset , int i)
{    int j;
     int save;

// TODO: This needs to be handled a level above and passed
//     if (depth <=0)                      /* try terminal, but not guaranteed */
//       i=random_T_czj();
//     else
//       i=random_FT_czj();                                      /* try either */
     ...
     if ( i < fset->function_count) 
     {
          child_list_lcl = (int *)MALLOC( (fset->cset)[i].arity * sizeof(int));
       	  // now spin local information to find all children of 'i'
   	  // TODO: FORCE THIS TO GENERATE TERMINALS IF WE ARE AT A CERTAIN DEPTH
   	  if (depth <= 1)
	  	spin_regrow_lcl1_information_terminals_mwh(i,child_list_lcl);
	  else
	  	spin_regrow_lcl1_information_mwh(i,child_list_lcl);
...
void generate_random_full_tree_wrapper_lcl1_mwh(int space, int depth, function_set *fset,const int source) 
/*
...
*/
{
	...
	if (source == REGROW_SOURCE) {
		// First generate root using czj root information
		if (depth <= 0)
			i=random_T_czj();
		else 
			i=random_F_czj();
		...
		{ case FUNC_DATA: case EVAL_DATA:
			...
			{ Function_czj = i;
			  Argument_czj = j;
			  generate_random_full_tree_wrapper_lcl1_mwh(space,depth-1,fset,WRAPPER_SOURCE);
			}
			break;
		   case FUNC_EXPR: case EVAL_EXPR:
		   	for (j = 0; j < (fset->cset)[i].arity; ++j)
	...
	if ( source == WRAPPER_SOURCE) {
		...
		// Depending on depth, try and generate the correct one
		// Not guaranteed according to czj notes in regrow/full tree functions
		if (depth <= 0)
		   i = random_T_czj();
		else
		   i = random_F_czj();
...
void generate_random_grow_tree_wrapper_lcl1_mwh(int space, int depth, function_set *fset,const int source) 
/*
 Depth is the depth left until the bottom
*/
{
	...
	if (source == REGROW_SOURCE) {
		...
		{ case FUNC_DATA: case EVAL_DATA:
			...
			{ Function_czj = i;
			  Argument_czj = j;
			  generate_random_grow_tree_wrapper_lcl1_mwh(space,depth-1,fset,WRAPPER_SOURCE);
			}
			break;
		   case FUNC_EXPR: case EVAL_EXPR:
		   	for (j = 0; j < (fset->cset)[i].arity; ++j)
...
void operator_regrow2Local ( population *oldpop, population *newpop,
                      void *data )
{
     int i;
     int ps;
