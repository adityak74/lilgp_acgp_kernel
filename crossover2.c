...
void generate_crossover_weights_lvl1_lcl1_for_lvl1_mwh(int trueIndex,int *context,int fixed)
/*
...

{
	...
	printf("In generate_crossover_weights_lvl1_lcl1_for_lvl1_mwh\n");
	...
	for (i = 0; i < farity; i++) {
		if (i != fixed)
			base_offset += MS_czj[trueIndex][i].convert_to_ms[ context[i] ]*Level1_Cnt_mwh.f[f].offset[i];
	}
...
#ifdef DEBUG_CROSSOVER
	printf("Showing weights for this legal mutation set,[");
	for (i = 0; i < size_fixed; i++) {
		printf("%f ",weights_for_this_MS[i]);
	}
	printf("]\n");
#endif
	...
	for (i = 0; i < MS_czj[trueIndex][fixed].numFT; i++) {
		crossover_weights_mwh[ MS_czj[trueIndex][fixed].members[i] ] = weights_for_this_MS[i];
	}

#ifdef DEBUG_CROSSOVER
	printf("Raw fset crossover weights are [");
	for (i = 0; i < numFT; i++) {
		printf("%f ",crossover_weights_mwh[i]);
	}
	printf("]\n");
#endif
...
void generate_crossover_weights_lvl1_lcl1_for_lcl1_mwh(int trueIndex,int *context,int fixed)
/*
   'context' is an array of children of the root 'trueIndex'
   'fixed' is the part that we are crossing at, basically fixed in that
   we do not consider it as part of our context
...

{
...
#ifdef DEBUG_CROSSOVER
	printf("In generate_crossover_weights_lvl1_lcl1_for_lcl1_mwh\n");
	printf("Parent of our context is %d,fixed index is %d\n",f,fixed);
	...
	//printf("Number of possible crossover labels = %d\n",size_fixed);
	...
	for (i = 0; i < farity; i++) {
		if (i != fixed)
			base_offset +=  MS_czj[trueIndex][i].convert_to_ms[ context[i] ]*Local1_mwh.f[f].offset[i];
	}

#ifdef DEBUG_CROSSOVER	
	printf("Local weights wheel for %d is [",f);
	for (i = 0; i < Local1_mwh.f[f].num_elems; i++) {
		printf("%f ",Local1_mwh.f[f].prob[i]);
	}
	printf("]\n");
#endif	
	...
	for (i = 0; i < size_fixed; i++) {
		...
		//printf("Offset to get weight for weights_for_this_MS is %d\n",this_offset);	
	...
	// now set the nonzero elements using our weights_for_this_MS
	for (i = 0; i < MS_czj[f][fixed].numFT; i++) {
		crossover_weights_mwh[ MS_czj[f][fixed].members[i] ] = weights_for_this_MS[i];
	}	

#ifdef DEBUG_CROSSOVER	
	//Display for debugging the list of crossover weights
	printf("Crossover weights are [");
	for (i = 0; i < numFT; i++) {
		printf("%f ",crossover_weights_mwh[i]);
	}
	printf("]\n");
...
int spin_after_generating_crossover_weights_lvl1_lcl1_mwh(lnode *l, int size_tree,int trueIndex, int fixed, int intExt) 
/*
...
*/
{
	...
	lnode *root = l;
        generate_crossover_wheel_recurse_lvl1_lcl1(&l,crossover_wheel,&cur_node,trueIndex,fixed,&total_assigned,intExt);

	// Retry but this time allow anything
	if (total_assigned == 0) {
		intExt = 0;
		cur_node = 0;
		total_assigned = 0;
		// Reset our pointer back to the root
		l = root;
		generate_crossover_wheel_recurse_lvl1_lcl1(&l,crossover_wheel,&cur_node,trueIndex,fixed,&total_assigned,intExt);
	}
...
#ifdef DEBUG_CROSSOVER	
	printf("Probabilities generated by crossover is [");
...
#ifdef DEBUG_CROSSOVER	
	printf("Wheel generated by crossover is [");
	for (i = 0; i < size_tree; i++) {
		printf("%f ",crossover_wheel[i]);
	}
	printf("]\n");
...
int generate_crossover_wheel_recurse_lvl1_lcl1( lnode **l,double *crossover_wheel, int *cur_node , int trueIndex, int fixed,int *assigned, int intExt)
{
     function *parent;
    ...
    //printf("Parent index is %d\n",parentIndex);
     ...
     // If it is a minweight, do not allocate any size for it
     if (weight <= L2minweight + SMALL)
     	weight = 0;
     //printf("cur_node=%d\n",*cur_node);
     if (intExt == 0) {
	...
	if (weight > SMALL) {
     ...
     else if (parentIndex >= fset->function_count) {
    	 	// its a terminal
		if (intExt == 2) {
   	  		crossover_wheel[*cur_node] = weight;
			if (weight > SMALL) 
...
#ifdef DEBUG_CROSSOVER
	printf("Node=%d fsetIndex=%d,weight calculated = %f, weight assigned is =%f",*cur_node,parentIndex,weight,crossover_wheel[*cur_node]);
#endif	
	...
	*cur_node = *cur_node + 1;
     ...
     {  case FUNC_DATA:
        case EVAL_DATA:
          ...
          {    
               generate_crossover_wheel_recurse_lvl1_lcl1(l,crossover_wheel,cur_node,trueIndex,fixed,assigned, intExt);
		/* going back up */  
          }
          break;
        case FUNC_EXPR:
...
int operator_crossover2_init ( char *options, breedphase *bp )
{
     int errors = 0;
     crossover_data *cd;
     int i, j, k, m;
     ...
     if (p=get_parameter( "acgp.level")) {
      //   printf("We are using level 2 heuristics operator crossover2.\n");
         Acgp_level = atof(p);
     }
     else {
         Acgp_level = 1;
     ...
     if (Acgp_level < 2) {
     	printf("Trying to use level 2 heuristic operator(crossover2) when acgp_level is not greater than 1!\n");
	...
	exit(1);
     ...
     bp->operator = OPERATOR_CROSSOVER2;
     bp->data = (void *)cd;
     bp->operator_free = operator_crossover2_free;
     bp->operator_start = operator_crossover2_start;
     bp->operator_end = operator_crossover2_end;
     bp->operator_operate = operator_crossover2;
...
void operator_crossover2_free ( void *data )
{
     crossover_data * cd;
...
void operator_crossover2_start ( population *oldpop, void *data )
{
     crossover_data * cd;
...
void operator_crossover2_end ( void *data )
{
     crossover_data * cd;
...
void operator_crossover2 ( population *oldpop, population *newpop,
                         void *data )
{ crossover_data * cd;
  int p1, p2;
  int ps1, ps2;
  ...
  { 
    ...
    { l1 = random_int(ps1);
      ...
      if (ps1 > 1) {
	   ...
	   printf("Cntxt info is (%s)(",fset[0].cset[trueIndex].string);
  	 	...
  	 	printf("%d %s, ",level1context_mwh[i],fset[0].cset[level1context_mwh[i]].string);
      	   }
#endif
       ...
       printf("DEPTH SOURCE=%d,",replace_depth);
       if (replace_depth >= 1) {
       	printf("Cntxt info is (%s)(",fset[0].cset[trueIndex].string);
      	 for (i = 0; i < fset[0].cset[trueIndex].arity; i++) {
  		    printf("%d %s, ",level1context_mwh[i],fset[0].cset[level1context_mwh[i]].string);
    ...
    if (replace_depth >= 1) {
      ...
      int num_in_source;
      	...
      	if (forceany2)
      	   num_in_source = spin_after_generating_crossover_weights_lvl1_lcl1_mwh(oldpop->ind[p2].tr[t2].data,ps2,trueIndex,thisArity,0);
	else if (total*random_double() < cd->internal)
		num_in_source = spin_after_generating_crossover_weights_lvl1_lcl1_mwh(oldpop->ind[p2].tr[t2].data,ps2,trueIndex,thisArity,1);
	   else
  		num_in_source = spin_after_generating_crossover_weights_lvl1_lcl1_mwh(oldpop->ind[p2].tr[t2].data,ps2,trueIndex,thisArity,2);
      ...
      // Not doing depth 1, so lets do normal czj picking of source
  ...
  { 
	...
	// Only gather info if it has more than just a root, that is has depth 0
	 ...
	 // Now select our dest from our own wheel, so spin it again
     	  ...
     	  int num_in_source;
	  if (forceany1)
	     num_in_source = spin_after_generating_crossover_weights_lvl1_lcl1_mwh(oldpop->ind[p1].tr[t1].data,ps1,trueIndex,thisArity,0);
          else if (total*random_double() < cd->internal)
	     	num_in_source = spin_after_generating_crossover_weights_lvl1_lcl1_mwh(oldpop->ind[p1].tr[t1].data,ps1,trueIndex,thisArity,1);
	  	else
			num_in_source = spin_after_generating_crossover_weights_lvl1_lcl1_mwh(oldpop->ind[p1].tr[t1].data,ps1,trueIndex,thisArity,2);
           ...
           // isnt depth>=1, so use old method
     	   if (forceany1)
